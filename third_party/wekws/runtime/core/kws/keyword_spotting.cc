...
KeywordSpotting::KeywordSpotting(const std::string& model_path) {
  ...
  const char* num_layers =
      metadata.LookupCustomMetadataMap("num_layers", allocator);
  if (num_layers != nullptr) {
    num_layers_ = std::stoi(num_layers);
  }
  std::cout << "Kws Model Info:" << std::endl
            << "\tcache_dim: " << cache_dim_ << std::endl
            << "\tcache_len: " << cache_len_ << std::endl
            << "\tnum_layers: " << num_layers_ << std::endl;
  Reset();
}
...
void KeywordSpotting::Reset() {
  ...
  cache_.resize(cache_dim_ * cache_len_ * num_layers_, 0.0);
  const int64_t cache_shape[] = {1, cache_dim_, cache_len_, num_layers_};
  cache_ort_ = Ort::Value::CreateTensor<float>(memory_info, cache_.data(),
                                               cache_.size(), cache_shape, 4);
}
