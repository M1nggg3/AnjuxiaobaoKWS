...
class Fbank {
 public:
  Fbank(int num_bins, int sample_rate, int frame_length, int frame_shift)
    ...
    // Match torchaudio.compliance.kaldi.fbank default window_type="povey".
    povey_window_.resize(frame_length_);
    double a = M_2PI / (frame_length - 1);
    for (int i = 0; i < frame_length; i++) {
      double i_fl = static_cast<double>(i);
      povey_window_[i] = pow(0.5 - 0.5 * cos(a * i_fl), 0.85);
    }
  }
  ...
  // add povey window, same default as Kaldi fbank
